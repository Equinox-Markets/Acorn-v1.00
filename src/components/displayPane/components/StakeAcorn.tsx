...
const AcornStake: FC<AcornStakeProps> = ({ vault }) => {
  ...
  >
    ...
    <Card
      ...
      <h2 className="smaller-h2" style={{ textAlign: 'left', marginLeft: '15px' }}>ETH Rewards: {vaultTokenBalance.isZero() ? "0.0" : parseFloat(ethers.utils.formatUnits(vaultTokenBalance, decimals)).toFixed(4)}</h2>
      </div>
      <div style={{ display: 'flex', alignItems: 'center' }}>
      <Button
